...
async def test_create_and_delete_entry(auth_client: AnyStrapiClient, post3_attrs: dict) -> None:
    ...
    find_res: StrapiEntriesResponse = await asyncutils.value(
        auth_client.get_entries('posts', filters={"title": {"$eq": post3_attrs["title"]}}))
    if find_res['data']:
        # Delete if post already exists
        await asyncutils.value(auth_client.delete_entry('posts', find_res['data'][0]['id']))

    create_res: StrapiEntryResponse = await asyncutils.value(auth_client.create_entry('posts', post3_attrs))
    assert create_res['data']
    assert is_match(create_res['data']['attributes'], post3_attrs)
    delete_res: StrapiEntryResponse = await asyncutils.value(
...
async def test_delete_entry_forbidden(client: AnyStrapiClient) -> None:
    ...
    with pytest.raises(Exception):
        await asyncutils.value(client.delete_entry('posts', 2))
